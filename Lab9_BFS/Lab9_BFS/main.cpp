 ...
 * @group 30422
 *
 * Task: implement the following functions from bfs.cpp using the provoded framework
 * - get_neighbors
 * - bfs
 * - print_bfs_tree
 * - performance
 ...
 * -shortest_path
 * - show that a knight can end up anywhere on a chessboard and give an example of a
 * chessboard with unreachable positions
 *
 * Solutions:
 * - get_neighbors:
 * For directions exist for a step: up, down, left, right. I checked each of them and considered
 * correct all of those which resulted in a free cell inside the grid. O(1)
 * - bfs:
 * After having the graph build, bfs consists of visiting all the nodes exactly once, such that
 * nodes at distance k from the root are guaranteed to be visited before those at distance k+1.
 * To do so, nodes are colored such that white nodes are not visited, gray nodes are discovered
 * but not finished, and black nodes are finished.
 * Whenever a node becomes gray, it is inserted into the queue and when it is finished it is popped
 * from the queue. Before it getting finished, all of its undiscovered neighbors are inserted
 * into the queue too.
 * Because each node is colored gray respectively black exactly once, and each edge is checked twice
 * (from both directions), this results in an O(E+V) algorithm.
 * - print_bfs_tree: adjusted from lab assignment nr. 6, see that for more explanation
 * - performance: for generating a random graph I first generated a random tree such that all
 * nodes were reachable from node 0, and than added the missing edges randomly taking into account only
 * that an edge should not be duplicated.
 * - shortest_path: I adjusted bfs so that it was running only until the target was reached. Then, from the
 * parent tree I reconstructed the shortest path defines by dfs by going step by step from node to its parent.
 * - knight bfs: I have overriden the function for finding the neighbors of a cell. By running bfs on
 * the input file grid_chess.txt we can see that all cells are reachable.
 * In a chess board of 3*3 the middle cell would not be reachable by a knight. In a 4*4 cell they are all reachable,
 * and from this point on we can prove by induction that in any n*n board all cells are reachable (with (0, 0) as root)
 * (Inductive step (n -> n+1):
 * For n+1 * n+1, root (0, 0), all cells with coordinates smaller than n+1 are reachable using the hypothesis.
 * Then, cell (0, 1) was reachable, so considering the upper-right n*n square, this cell is its (0, 0) root, and
 * all of its cells are reachable using the hypothesis.
 * So at this point we know that all cells in the first n rows are reachable.
 * By repeating this idea for the lower-left and lower-right n*n squares we can complete the proof.
 *
 * Performance evaluation of bfs:
 * The resulting charts correspond to our expectations: in both cases, when having constant nr. of edges and varying
 * nr of nodes and vice-versa, we can see a linear growth, meaning that the algorithm complexity is linear
 * both in terms of V and E. More than that, the total nr. of operations corresponds approximately to E+V.
 * So, indeed, the algorithm is O(E+V).
 * 
 */
